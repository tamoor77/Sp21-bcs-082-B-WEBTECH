...
//vehicle
router.get('/vehicl', async (req, res) => {
  try {
    const response = await axios.get('http://localhost:8081/vehicles');
    const vehicles = response.data;
    res.render('pages/index', { vehicles, error: null });
  } catch (err) {
    console.error(err);
    res.render('pages/index', { vehicles: [], error: 'Failed to fetch vehicles' });
  }
});
...
router.get('/adm', async (req, res) => {
  try {
    const response = await axios.get('http://localhost:8081/vehicles/admin');
    res.render('pages/admin', {
      vehicles: response.data,  // ✅ important: pass vehicles to the template
      error: null
    });
  } catch (err) {
    console.error(err);
    res.render('pages/admin', {
      vehicles: [],             // ✅ prevent "vehicles not defined" error
...
router.post('/vehicles/create', async (req, res) => {
  try {
    ...
    const response = await axios.post('http://localhost:8081/vehicles/admin', formData, {
      headers: {
    ...
    console.error(err);
    res.render('pages/newVehicle', { error: 'Failed to create vehicle' });
  }
});
...
router.get('/admin/vehicles', async (req, res) => {
  try {
    const response = await axios.get('http://localhost:8081/vehicles/admin');
    res.render('pages/admin', { vehicles: response.data });
  } catch (err) {
    console.error(err);
    res.render('pages/admin', { error: 'Failed to load vehicles', vehicles: [] });
  }
});
...
router.get('/vehicles/edit/:id', async (req, res) => {
  try {
    const response = await axios.get(`http://localhost:8081/vehicles/admin/edit/${req.params.id}`);
    res.render('pages/editVehicle', { vehicle: response.data });
  } catch (err) {
    console.error(err);
...
router.post('/vehicles/edit/:id', async (req, res) => {
  try {
    ...
    await axios.post(`http://localhost:8081/vehicles/admin/edit/${req.params.id}`, formData, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    });
...
router.post('/vehicles/delete/:id', async (req, res) => {
  try {
    await axios.post(`http://localhost:8081/vehicles/admin/delete/${req.params.id}`);
    res.redirect('/admin/vehicles');
  } catch (err) {
    console.error(err);
    res.redirect('/admin/vehicles');
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    req.session.user = {
  ...response.data.user,
  token: response.data.token   // ✅ Store the token correctly
};
...
router.post('/place-order', isAuthenticated, async (req, res) => {
  ...
  try {
    await axios.post('http://localhost:8081/api/v1/orders/order', {
      items,
      totalAmount
    }, {
      headers: {
...
router.get('/my-orders', isAuthenticated, async (req, res) => {
  try {
    const response = await axios.get('http://localhost:8081/api/v1/orders/order/my', {
      headers: {
        Authorization: `Bearer ${req.session.user?.token}`
      }
    });
